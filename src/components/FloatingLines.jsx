...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 基础色固定为黑（项目始终传入 linesGradient，不再走旧的蓝粉渐变背景路径）
  vec3 b = vec3(0.0);
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // 降负载：关闭 antialias（着色器本身线条柔和，视觉差别几乎不可感知）
    const renderer = new WebGLRenderer({ antialias: false, alpha: false });
    // 降负载：渲染分辨率上限 DPR 2 → 1.25，大幅减少片元着色计算量
    ...
    let raf = 0;
    // 页面隐藏时暂停 rAF 渲染省电，重新可见时恢复
    // （iTime 基于 clock.getElapsedTime()，恢复瞬间会跳变，视觉上可接受）
    let paused = document.visibilityState === 'hidden';
    // 降负载：渲染降帧 60fps → 30fps，距上次实际渲染不足帧间隔时跳过本帧的
    // uniform 更新与 renderer.render，但仍挂下一帧 rAF 保证循环不断；
    // iTime 仍取 clock.getElapsedTime()，跳帧时动画总时长连续、不会卡顿回跳
